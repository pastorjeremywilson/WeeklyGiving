...
class Table(Flowable):
                ...
                for j,(v, s, w) in enumerate(list(zip(V, S, W))): # value, style, width (lengths must match)
                    ...
                    if ji in rowSpanCells and not span:
                        ...
                        if isinstance(v,(tuple,list,Flowable)):
                            ...
                            if canv: canv._fontname, canv._fontsize, canv._leading = s.fontname, s.font_size, s.leading or 1.2 * s.font_size
                            ...
                            t = (s.leading or 1.2 * s.font_size) * len(v)
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if self._curcellstyle is not cellstyle:
            ...
            if cur is None or cellstyle.leading != cur.leading or cellstyle.fontname != cur.fontname or cellstyle.font_size != cur.fontsize:
                self.canv.setFont(cellstyle.fontname, cellstyle.font_size, cellstyle.leading)
...
def _setCellStyle(cellStyles, i, j, op, values):
    ...
    if op == 'FONT':
        ...
        if n>1:
            new.font_size = values[1]
            if n>2:
                new.leading = values[2]
            else:
                new.leading = new.font_size * 1.2
    elif op in ('FONTNAME', 'FACE'):
        new.fontname = values[0]
    elif op in ('SIZE', 'FONTSIZE'):
        new.font_size = values[0]
    elif op == 'LEADING':
